...
/**
 * Resolve column metadata for a header. Handles both:
 *   - Prefixed: `table__column`
 *   - Unprefixed join key: `student_id`
 */
...
export const validateRows = async ({ template, rows, uploadId, chunkId }) => {
  if (!template?.metadata?.columnsByTable) {
    throw new Error('Template metadata missing column definition');
  }
