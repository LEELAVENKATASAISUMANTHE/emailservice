...
export const uploadBuffer = async (objectName, buffer, metadata = {}) => {
  await minioClient.putObject(config.minio.bucket, objectName, buffer, buffer.length, metadata);
  return objectName;
};

export const getObjectStream = async (objectName) => minioClient.getObject(config.minio.bucket, objectName);
